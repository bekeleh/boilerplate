                        ...
                        <div class="mt-1">
                            ...
                            >
                                <span slot="noResult" class="text-gray-700">
                                    Oops! No elements found. Consider changing
                                    the search query.
                                </span>
                                >
                            </VueMultiselect>
...
export default {
    ...
    methods: {
        fetchServiceApprover() {
            this.$store
                .dispatch(
                    "adminServiceApprover/fetchServiceApproverById",
                    this.$route.params.id
                )
                .then(
                    (response) => {
                        this.service = response.data.data;
                        this.selectedServices = response.data.data.service;
                        this.selectedRequirements =
                            response.data.data.requirements;
                        this.selectedRequisitions =
                            response.data.data.requisitions;
                        this.selectedApprovers = response.data.data.approvers;
        ...
        onEditSubmit(values, actions) {
            const data = {
                ...
                category_id: this.selectedServices
                    ? this.selectedServices.id
                    : null,
                approver_id: this.selectedApprovers
                    ? this.selectedApprovers.id
                    : null,
            ...
            this.loading = true;
            this.$store
                .dispatch("adminServiceApprover/update", {
                    id: this.service.id,
                    data: data,
                })
                .then(
