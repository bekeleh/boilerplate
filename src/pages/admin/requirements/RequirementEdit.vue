...
export default {
    ...
    methods: {
        ...
        fetchRequirement() {
            this.$store
                .dispatch(
                    "adminRequirement/fetchRequirementById",
                    this.$route.params.id
                )
                .then(
                    (response) => {
                        this.requirement = response.data.data;
                        this.requirement.is_required = this.requirement
                            .is_required
                            ? true
                            : false;
                        this.requirement.is_active = this.requirement.is_active
                            ? true
                            : false;
                    },
                    (error) => {}
        ...
        onEditSubmit(values, actions) {
            ...
            this.$store
                .dispatch("adminRequirement/update", {
        ...
        removeAttachment(name) {
            this.$store.dispatch("adminTicket/removeAttachment", name).then(
                (response) => {
                    this.loading = false;
                    if (response.data && response.data.success) {
                        this.fetchRequirement();
