            ...
            >
                ...
                >
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody v-if="requirements.length">
                            <tr
                                v-for="(
                                    requirement, requirementIdx
                                ) in requirements"
                                :key="requirementIdx"
                                :class="
                                    requirementIdx % 2 === 0
                            ...
                            >
                                ...
                                >
                                    <a
                                        v-if="can('view', 'requirement')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="
                                            viewRequirement(requirement.id)
                                        "
                                        >View</a
                                    >
                                    <a
                                        v-if="can('edit', 'requirement')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="
...
export default {
    ...
    methods: {
        ...
        Filter() {
            if (!this.disableSearchBtn) this.fetchRequirement();
        },
        newRequirement() {
            this.$router.push({ name: "admin.requirements.new" });
        },
        editRequirement(id) {
            this.$router.push({
        ...
        deleteRequirement(id) {
            this.$store
                .dispatch("adminRequirement/delete", id)
                .then((response) => {
        ...
        fetchRequirement() {
            this.loading = true;
            this.$store
                .dispatch("adminRequirement/fetchRequirement", {
        ...
        onChange(url) {
            ...
            this.$store.dispatch("adminRequirement/paginate", query).then(
                (response) => {
                    this.requirements = response.data.data;
                    this.pagination.links = response.data.links;
