                    ...
                    >
                        ...
                        >
                            <div v-if="can('create', 'user')">
                                <button
                                    type="button"
                                    class="order-0 mb-2 inline-flex items-center pr-4 pl-3 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-et-green-light hover:bg-et-green focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-et-green sm:order-1 sm:ml-3"
                                    @click="newRole"
                                >
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody v-if="users.length">
                            ...
                            >
                                ...
                                <td
                                    class="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-700"
                                >
                                ...
                                <td
                                    class="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-500"
                                >
                                    {{ fromNow(user.email_verified_at) }}
                                </td>
                                <td
                                    class="px-3 py-2 whitespace-nowrap text-sm text-gray-500"
                                >
                                    {{ fromNow(user.last_logged_in_at) }}
                                </td>
                                <td
                                    class="px-3 py-2 whitespace-nowrap text-sm text-gray-500"
                                >
...
export default {
    ...
    methods: {
        ...
        Filter() {
            if (!this.disableSearchBtn) this.fetchUser();
        },
        newRole() {
            this.$router.push({
        ...
        deleteUser(id) {
            this.$store.dispatch("adminUser/delete", id).then((response) => {
        ...
        onChange(url) {
            ...
            this.$store.dispatch("adminUser/paginate", query).then(
                (response) => {
                    this.users = response.data.data;
                    this.pagination.links = response.data.links;
