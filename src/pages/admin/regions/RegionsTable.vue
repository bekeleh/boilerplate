                ...
                >
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody v-if="regions.length">
                            <tr
                                v-for="(region, regionIdx) in regions"
                                :key="regionIdx"
                                :class="
                                    regionIdx % 2 === 0
                                        ? 'bg-white'
                            ...
                            >
                                ...
                                >
                                    {{
                                        region.is_active ? "inactive" : "active"
                                    }}
                                </td>
                                <td
                                    class="px-3 py-2 whitespace-nowrap text-sm text-gray-500"
                                ...
                                >
                                    <a
                                        v-if="can('view', 'region')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="viewRegion(region.id)"
                                        >View</a
                                    >
                                    <a
                                        v-if="can('edit', 'region')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="editRegion(region.id)"
                                        >Edit</a
                                    >
...
export default {
    ...
    methods: {
        ...
        Filter() {
            if (!this.disableSearchBtn) this.fetchRegion();
        },
        newRegion() {
            this.$router.push({ name: "admin.regions.new" });
        },
        editRegion(id) {
            this.$router.push({
        ...
        deleteRegion(id) {
            this.$store.dispatch("adminRegion/delete", id).then((response) => {
        ...
        fetchRegion() {
            this.loading = true;
            this.$store
                .dispatch("adminRegion/fetchRegion", {
        ...
        onChange(url) {
            ...
            this.loading = true;
            this.$store.dispatch("adminRegion/paginate", query).then(
                (response) => {
                    this.regions = response.data.data;
                    this.pagination.links = response.data.links;
