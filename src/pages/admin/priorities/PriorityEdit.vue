...
export default {
    ...
    created() {
        ...
        this.fetchPriority(this.id);
    },
    methods: {
        fetchPriority(id) {
            this.$store.dispatch("adminPriority/fetchPriorityById", id).then(
                (response) => {
                    this.priority = response.data.data;
                    this.enabled = this.priority.is_active ? true : false;
                },
                (error) => {}
            );
        },
        ...
        handleCancel() {
            this.$router.push({ name: "admin.priorities" });
        },
    },
};
</script>
