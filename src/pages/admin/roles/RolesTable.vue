...
export default {
    ...
    methods: {
        ...
        Filter() {
            if (!this.disableSearchBtn) this.fetchRole();
        },
        newRole() {
            this.$router.push({ name: "admin.roles.new" });
        },
        editRole(id) {
            this.$router.push({ name: "admin.roles.edit", params: { id: id } });
        },
        viewRole(id) {
            this.$router.push({ name: "admin.roles.view", params: { id: id } });
        },
        deleteRole(id) {
            this.$store.dispatch("adminRole/delete", id).then((response) => {
        ...
        onChange(url) {
            ...
            this.loading = true;
            this.$store.dispatch("adminRole/paginate", query).then(
                (response) => {
                    this.roles = response.data.data;
                    this.pagination.links = response.data.links;
