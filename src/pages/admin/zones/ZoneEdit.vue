...
export default {
    ...
    created() {
        ...
        this.fetchZone(this.id);
    },
    methods: {
        fetchZone(id) {
            this.$store.dispatch("adminZone/fetchZoneById", id).then(
                (response) => {
                    this.zone = response.data.data;
                    this.selectedRegion = response.data.data.regions;
                    this.enabled = this.zone.is_active ? true : false;
                },
                (error) => {}
            );
        },
        onEditSubmit(values, actions) {
            const data = {
                ...
                region_id: this.selectedRegion[0]
                    ? this.selectedRegion[0].id
        ...
        createService(searchQuery) {
            ...
            axios.post(`/api/v1/region_options`, params).then((res) => {
                this.selectedRegion.push(res.data.data);
            });
        },
    },
};
