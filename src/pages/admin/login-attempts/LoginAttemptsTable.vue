            ...
            >
                ...
                >
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody v-if="login_attempts.length">
                            <tr
                                v-for="(
                                    login_attempt, login_attemptIdx
                                ) in login_attempts"
                                :key="login_attemptIdx"
                                :class="
                                    login_attemptIdx % 2 === 0
                                        ? 'bg-white'
                                        : 'bg-gray-50'
                                "
...
export default {
    ...
    methods: {
        ...
        Filter() {
            if (!this.disableSearchBtn) this.fetchLoginAttempt();
        },
        newLoginAttempt() {
            this.$router.push({
                name: "admin.login_attempts.new",
            });
        },
        editLoginAttempt(id) {
            this.$router.push({
                name: "admin.login_attempts.edit",
                params: { id: id },
            });
        },
        viewLoginAttempt(id) {
        ...
        deleteLoginAttempt(id) {
            this.$store
                .dispatch("adminLoginAttempt/delete", id)
                .then((response) => {
        ...
        fetchLoginAttempt() {
            this.loading = true;
            this.$store
                .dispatch("adminLoginAttempt/fetchLoginAttempt", {
        ...
        onChange(url) {
            ...
            this.$store.dispatch("adminLoginAttempt/paginate", query).then(
                (response) => {
                    this.login_attempts = response.data.data;
                    this.pagination.links = response.data.links;
