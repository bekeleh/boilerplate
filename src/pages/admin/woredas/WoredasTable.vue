                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody v-if="zones.length">
                            ...
                            >
                                ...
                                >
                                    {{ zone.is_active ? "inactive" : "active" }}
                                </td>
                                <td
                                    class="px-3 py-2 whitespace-nowrap text-sm text-gray-500"
                                >
                                ...
                                >
                                    <a
                                        v-if="can('view', 'woreda')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="viewWoreda(zone.id)"
                                        >View</a
                                    >
                                    <a
                                        v-if="can('edit', 'woreda')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="editWoreda(zone.id)"
                                        >Edit</a
                                    >
                                    <a
                                        v-if="can('delete', 'woreda')"
                                        href="#"
                                        class="text-indigo-600 mr-2 hover:text-indigo-900 cursor-pointer"
                                        @click.stop="deleteWoreda(zone.id)"
                                        >Delete</a
                                    >
                                </td>
                            </tr>
...
export default {
    ...
    methods: {
        ...
        Filter() {
            if (!this.disableSearchBtn) this.fetchWoreda();
        },
        newWoreda() {
            this.$router.push({ name: "admin.woredas.new" });
        },
        editWoreda(id) {
            this.$router.push({
        ...
        deleteWoreda(id) {
            this.$store.dispatch("adminWoreda/delete", id).then((response) => {
        ...
        fetchWoreda() {
            this.loading = true;
            this.$store
                .dispatch("adminWoreda/fetchWoreda", {
        ...
        onChange(url) {
            ...
            this.loading = true;
            this.$store.dispatch("adminWoreda/paginate", query).then(
                (response) => {
                    this.zones = response.data.data;
                    this.pagination.links = response.data.links;
