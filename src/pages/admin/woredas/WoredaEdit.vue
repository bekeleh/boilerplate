...
export default {
    ...
    created() {
        ...
        this.fetchWoreda(this.id);
    },
    methods: {
        fetchWoreda(id) {
            this.$store.dispatch("adminWoreda/fetchWoredaById", id).then(
                (response) => {
                    this.woreda = response.data.data;
                    this.selectedZone = response.data.data.zones;
                    this.enabled = this.woreda.is_active ? true : false;
                },
                (error) => {}
            );
        },
        onEditSubmit(values, actions) {
            const data = {
                ...
                zone_id: this.selectedZone[0]
                    ? this.selectedZone[0].id
        ...
        handleCancel() {
            this.$router.push({ name: "admin.woredas" });
        },
        loadZone(search) {
            const params = {
                search: search,
            ...
            if (this.zoneTimeout) {
                clearInterval(this.zoneTimeout);
            }
            this.loading = true;
            this.zoneTimeout = setTimeout(() => {
                axios
                    .get(`/api/v1/search_zone_options`, { params })
                    .then((res) => {
        ...
        createZone(searchQuery) {
            const params = {
                search: searchQuery,
            };
            axios.post(`/api/v1/search_zone_options`, params).then((res) => {
                this.selectedZone.push(res.data.data);
            });
        },
    },
};
