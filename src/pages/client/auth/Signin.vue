    ...
    <div class="min-h-full flex flex-col justify-center py-6 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <h2 class="mt-2 text-center text-2xl font-extrabold text-gray-600">
                Welcome to Ethio telecom VAS partners portal
            </h2>
        </div>
        <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-4 px-4 shadow sm:rounded-lg sm:px-10">
                <VeeForm @submit="onSubmit">
                    ...
                    <div>
                        <label
                            class="font-medium block text-sm text-gray-700"
                            for="password"
                        >
                            Password
                        </label>
                ...
                <div class="text-center mt-4">
                    ...
                    <!-- <p
                        v-if="client && !client.email_verified_at"
                        class="block mt-3 text-sm text-gray-600"
                    >
                        <a
                            class="no-underline text-et-green text-xl font-bold hover:text-et-green-light cursor-pointer"
...
export default {
    ...
    methods: {
        ...
        handleEmailResend() {
            this.loading = true;
            this.$store.dispatch("clientAuth/emailResend").then(
                (response) => {
                    this.loading = false;
                    this.show_alert = true;
                    if (response.data.success) {
                        this.redirect("client.email.verification");
                    }
                    if (!response.data.success) {
        ...
        onSubmit(values) {
            ...
            this.loading = true;
            this.$store.dispatch("clientAuth/login", user).then(
                (response) => {
                    this.loading = false;
                    this.show_alert = true;
                    this.alert_message = "";
